...
class RecommendationEngine:
    ...
    def get_recommendations(self, candidate_id: int, 
        ...
        # Filter out applied internships (pending and accepted) but include withdrawn ones
        ...
        if len(filtered_internships) < top_n:
            logger.info(f"Only {len(filtered_internships)} active internships available, trying inactive ones")
            inactive_internships = self.db.get_all_internships(active_only=False)
            for internship in inactive_internships:
                # Check if not already in filtered list
                if not any(f['id'] == internship['id'] for f in filtered_internships):
                    ...
                    if self.db.is_internship_accepted(candidate_id, internship['id']):
                        logger.info(f"Permanently excluding accepted inactive internship {internship['id']} for candidate {candidate_id}")
                        continue
                    ...
                    if len(filtered_internships) >= top_n * 2:  # Get 2x what we need for better selection
                        break
        ...
        # If we have fewer recommendations than requested, try to get more from lower scores
        if len(top_recommendations) < top_n and len(recommendations) > top_n:
            # Get additional recommendations from the remaining pool
            additional_needed = top_n - len(top_recommendations)
